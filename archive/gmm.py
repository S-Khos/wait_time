...
class GMM:
    """
    Gaussian Mixture Model implementation for lane clustering s
    """
    ...
    def __init__(
        ...
        self.initial_means = initial_means.copy() if initial_means is not None else None
    ...
    def _initialize_params(self, X):
        ...
        if self.means is None:
            ...
            #############################################
        self.covariances = [
            np.cov(X, rowvar=False) + self.epsilon * np.eye(X.shape[1])
    ...
    def _covariance(self, matrix):  # Hand coded helper function instead of np.cov
        ...
        return divided_covariances + (
            np.eye(matrix.shape[1]) * self.epsilon
    ...
    #         )  # Update the mixing coefficients (priors)
    #         self.means[i] = (
    #             np.dot(responsibilities[:, i], X)
    #             + self.reg_factor * self.initial_means[i]
    #         ) / (
    #             sum_responsibilities[i] + self.reg_factor
    #         )  # Regularize the update of the means
    ...
    def _m_step(self, X, responsibilities):
        ...
        for i in range(self.k):
            self.pi[i] = sum_responsibilities[i] / X.shape[0]
            self.means[i] = (
                np.dot(responsibilities[:, i], X)
                + self.reg_factor * self.initial_means[i]
            ) / (sum_responsibilities[i] + self.reg_factor)
            ...
            for j in range(X.shape[0]):
                outer_product = np.outer(diff[j], diff[j])
                cov += responsibilities[j, i] * outer_product
            cov /= sum_responsibilities[i]

            # Enforce minimum eigenvalue threshold
            eigvals, eigvecs = np.linalg.eigh(cov)
            eigvals[eigvals < self.min_eigenvalue] = self.min_eigenvalue
            scaled_cov = eigvecs @ np.diag(eigvals) @ eigvecs.T

            # Apply covariance scaling
            self.covariances[i] = (
                self.covariance_scale * scaled_cov + self.epsilon * np.eye(X.shape[1])
            )
...
def dfs(matrix, visited, i, j, label, coordinates):
    radius = 60  # Set the radius to 10
        ...
        for j in range(len(matrix[0])):
            if matrix[i][j] == 1 and not visited[i][j]:
                ...
                if len(coordinates) < 20:
                    # If the component is a single element, set it to 0 (anomaly removal)
                    x, y = coordinates[0]
                    matrix[x][y] = 0
                else:
                    # Label the component
...
rows, cols = 2592, 1944
# Create a 2D matrix filled with 0s
matrix = np.zeros((rows, cols), dtype=int)
# Populate the matrix with 255s at the given coordinates
for i, j in X:
...
print(initial_means)
# plot initial means on the matrix
matrix_ = matrix.copy()
# replace any element in 2d matrix that is not 0 with 255
matrix_[matrix_ != 0] = 255
plt.figure(figsize=(15, 10))
plt.title("Matrix with Initial Means", fontsize=16)
plt.imshow(matrix_, cmap="viridis", interpolation="nearest")
