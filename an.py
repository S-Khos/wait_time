...
class BoundingBoxData:
    def __init__(self, obj_id: str, left: float, top: float, right: float, bottom: float, timestamp: str = None, obj_class: str = None):
        self.obj_id = obj_id
        self.left = left
        self.top = top
        self.right = right
    ...
    for analytics in xml_root.xpath('.//tt:VideoAnalytics', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
        for frame in analytics.xpath('.//tt:Frame', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
            ...
            for obj in frame.xpath('.//tt:Object', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
                ...
                # Try to get class from Appearance/Class element
                for appearance in obj.xpath('.//tt:Appearance', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
                    for class_elem in appearance.xpath('.//tt:Class', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
                        class_type = class_elem.get('Type')
                        if class_type:
                ...
                # Alternative: try to get from Classification element
                if obj_class == "Unknown":
                    for classification in obj.xpath('.//tt:Classification', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
                        class_type = classification.get('Type') or classification.get('Class')
                        if class_type:
                            obj_class = class_type
                            break
                
                # Alternative: try to get from direct Type attribute
                if obj_class == "Unknown":
                ...
                for bbox in obj.xpath('.//tt:BoundingBox', namespaces={'tt': 'http://www.onvif.org/ver10/schema'}):
                    try:
                        ...
                        box = BoundingBoxData(obj_id, left, top, right, bottom, frame_timestamp, obj_class)
                        boxes.append(box)
                        
                        print(f"📦 Object {obj_id} [{obj_class}]: ({left:.3f},{top:.3f}) to ({right:.3f},{bottom:.3f})")
...
def draw_bounding_boxes(frame: np.ndarray, boxes: List[BoundingBoxData], overlay: MetadataOverlay) -> np.ndarray:
    ...
    for box in boxes:
        ...
        print(f"🔍 Box {box.obj_id} [{box.obj_class}]: ONVIF=({box.left:.3f},{box.top:.3f},{box.right:.3f},{box.bottom:.3f}) -> Original=({orig_x1:.1f},{orig_y1:.1f},{orig_x2:.1f},{orig_y2:.1f}) -> Scaled=({x1},{y1},{x2},{y2})")
        ...
        box_color = color_map.get(box.obj_class, (0, 255, 0))  # Default to green
        
        # Draw bounding box
        cv2.rectangle(frame, (x1, y1), (x2, y2), box_color, 2)
        
        # Draw labels with background
        label_id = f"ID: {box.obj_id}"
        label_class = f"Class: {box.obj_class}"
        ...
        (id_w, id_h), _ = cv2.getTextSize(label_id, font, font_scale, thickness)
        (class_w, class_h), _ = cv2.getTextSize(label_class, font, font_scale, thickness)
        
        # Draw background rectangles for labels
        label_bg_color = (0, 0, 0)
        cv2.rectangle(frame, (x1, y1 - id_h - class_h - 10), (x1 + max(id_w, class_w) + 4, y1), label_bg_color, -1)
        
        # Draw text labels
        cv2.putText(frame, label_id, (x1 + 2, y1 - class_h - 6), 
                   font, font_scale, (255, 255, 255), thickness)
        cv2.putText(frame, label_class, (x1 + 2, y1 - 2), 
                   font, font_scale, (255, 255, 255), thickness)
